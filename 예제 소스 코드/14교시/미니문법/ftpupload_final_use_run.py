...
run('c:\\\"Program Files\"\\7-Zip\\7z a -r -tzip c:\\python\\code\\zipfile\\backup_\"%DATE:~0,4%%DATE:~5,2%%DATE:~8,2%\".zip c:\\python\\code\source\\*.txt c:\\python\\code\\source\\*.jpg', shell=True) 
print ("zip done")  
...
run('forfiles /p "c:\\python\\code\\zipfile " /s /m backup*.zip /d -7 /c "cmd /c del @path"', shell=True)
print ("del done")
